...
let dbWavelet = Array(createWaveletFromCoefficients(db6, levels: 6).dropLast())
QuickArrayPlot(array: dbWavelet)

let dbReverse = Array(createWaveletFromCoefficients(db6.reversed(), levels: 6).dropLast())
QuickArrayPlot(array: dbReverse)

let waveletLength = 5
let waveletCount = 6
let o = dbWavelet.count / waveletLength
let plotLength = waveletLength + (waveletCount-1)
let viewLength = plotLength + 1
...
let viewValues = o * plotLength
let xArray = Array(0..<viewValues).map({Float(plotLength*$0)/Float(viewValues)})


let f = xArray.map({0.5*pow(0.75*$0-2.5, 2) - 2})
...
p0p.plotView.writeAsPdfTo(path: "/Users/vincentherrmann/Documents/Projekte/Wavelets/db6WaveletF0.pdf")
...
d1p.plotView.writeAsPdfTo(path: "/Users/vincentherrmann/Documents/Projekte/Wavelets/db6Synth1.pdf")
...
d2p.plotView.writeAsPdfTo(path: "/Users/vincentherrmann/Documents/Projekte/Wavelets/db6Synth2.pdf")
...
d3p.plotView.writeAsPdfTo(path: "/Users/vincentherrmann/Documents/Projekte/Wavelets/db6Synth3.pdf")
...
d4p.plotView.writeAsPdfTo(path: "/Users/vincentherrmann/Documents/Projekte/Wavelets/db6Synth4.pdf")
...
d5p.plotView.writeAsPdfTo(path: "/Users/vincentherrmann/Documents/Projekte/Wavelets/db6Synth5.pdf")
print("finished")
